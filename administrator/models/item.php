...
class CatalogueModelItem extends JModelAdmin
{
	...
	public function getItem($pk = null)
	{
		...
		else
		{
			...
			if (isset($this->_item->images) && !empty($this->_item->images))
			{
				$registry = new Registry;
			...
			$this->_item->itemtext = trim($this->_item->fulltext) != '' ? $this->_item->introtext . "<hr id=\"system-readmore\" />" . $this->_item->fulltext : $this->_item->introtext;

			if (isset($this->_item->similar_items))
			{
				$similar_items = new Registry;
				$similar_items->loadString($this->_item->similar_items);
				$this->_item->similar_items = $similar_items->toArray();
			}

			if (isset($this->_item->assoc_items))
			{
				$assoc_items = new Registry;
				$assoc_items->loadString($this->_item->assoc_items);
				$this->_item->assoc_items = $assoc_items->toArray();
			}
			...
			$this->_item->attrs = $this->_db->loadAssocList('attr_id');
			...
			$this->_item->attrdirs = $this->_db->loadObjectList();

			$query = $this->_db->getQuery(true);

			$query->select('p.attr_id, p.attr_price')
				->from('#__catalogue_attr_price as p')
				->where('p.item_id = ' . (int) $this->_item->id);

			$this->_db->setQuery($query);

			$attr_prices = $this->_db->loadAssocList('attr_id');

			foreach ($this->_item->attrdirs as $attr_dir)
			{
				if (isset($this->_item->params['attr_' . $attr_dir->attr_id]))
				{
					$attr_dir->attr_value = $this->_item->params['attr_' . $attr_dir->attr_id];
				}
			...
			$this->_item->attrs = JArrayHelper::toObject($this->_item->attrs);
			...
			if ($assoc)
			{
				$this->_item->associations = array();
				if ($this->_item->id != null)
				{
					$associations = JLanguageAssociations::getAssociations('com_catalogue', '#__catalogue_item', 'com_catalogue.item', $this->_item->id);
					foreach ($associations as $tag => $association)
					{
						$this->_item->associations[$tag] = $association->id;
					}
				}
			}
		}
